...
object Hash {
  ...
  def sha1Padding(data: Seq[Byte]): Seq[Byte] = {
    val ml = data.length
    val padLen = ((64 + 56) - ((ml + 1) % 64)) % 64
    0x80.toByte +: (Seq.fill[Byte](padLen)(0) ++ intTo64Bit(ml*8))
  }
  ...
  def sha1(data: Seq[Byte]): Seq[Byte] = {
    ...
    def calcChunk(chunk: Seq[Byte], h: Seq[Int]) = {
    ...
    val shaBytes = paddedData
      .grouped(64)
      .foldLeft(h){case (hVal, chunk) => calcChunk(chunk, hVal)}
      .flatMap(intTo32Bit)
    shaBytes
  }
